...
- **Supabase** (`@supabase/supabase-js`) — RSVP 데이터 저장
...
src/
├── app/
│   ├── api/rsvp/route.ts    # RSVP POST 핸들러 (Supabase insert)
...
└── components/
    ...
    ├── ScrollToTop.tsx          # 우측 하단 맨위로 버튼 (CoverSection에서는 숨김)
    ...
    ├── RSVPSection.tsx          # 참석 여부 폼 → /api/rsvp POST
    ├── GuestbookSection.tsx     # 축하 메시지 방명록 (현재 page.tsx에서 주석처리)
...
- NAV_ITEMS: `#event`, `#gallery`, `#directions`, `#account`, `#rsvp` (5개, `#guestbook`은 주석처리)
...
- CoverSection(`minHeight: 100svh`)에 있을 때는 숨김 (`scrollY > window.innerHeight * 0.8` 기준)
- 그 외 구간에서는 항상 표시, `opacity` 트랜지션으로 부드럽게 전환

## RSVPSection + Supabase

- 폼 항목: 이름(필수), 연락처, 구분(신랑/신부측), 참석여부, 참석인원 — 식사선택 없음
- 제출 시 `POST /api/rsvp` → `src/app/api/rsvp/route.ts` → Supabase `rsvp` 테이블 insert
- 환경변수 (`.env.local`):
  - `SUPABASE_URL`
  - `SUPABASE_ANON_KEY`
- Supabase RLS: anon에 insert만 허용 (`create policy "누구나 insert 가능" on rsvp for insert to anon with check (true)`)
- Supabase 테이블 스키마:
  ```sql
  create table rsvp (
